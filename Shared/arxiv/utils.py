...
def append_data(data, cur_pos, wrench, voltage, Joystick_inputs):
    time.time()
...
def play_shared_traj(conn, data, traj_name, voltage, total_time, alpha = 0.2):
    ...
    while True:
        ...
        wrench = state['O_F']
        scale = 0.4
        x_des[0] = (x_des[0]+scale*z[1])*alpha + x_des[0]*(1-alpha)
        x_des[1] = (x_des[1]+scale*z[0])*alpha + x_des[1]*(1-alpha)
        x_des[2] = (x_des[2]-scale*z[2]) * alpha + x_des[2] * (1 - alpha)
        ...
        if curr_t > total_time or wrench[2] < -10 or X_pressed:
            return data
        data = append_data(data, time.time(), state, voltage, Joystick_inputs)


def pick_object(obj, Objects):
    length = []
    # P(t|D) is higher if object distance to gripper is smaller
    if obj == 'soft':
        desired_pos = np.array([320, 0])
    if obj == 'rigid':
        desired_pos = np.array[[0, 0]]
    for idx in range(len(Objects['xc'])):
        x_l = Objects['xc'] - desired_pos[0]
        y_l = Objects['yc'] - desired_pos[1]
        length.append(np.linalg.norm([x_l, y_l]))

    # min_value = min(length)
    min_index = np.argmin(length)
    r1 = Objects['xc'][min_index]
    c1 = Objects['yc'][min_index]
    return r1, c1


def get_target(obj, cur_pos):
    pipeline, depth_scale = configure_camera()
    align_to = rs.stream.color
    align = rs.align(align_to)

    try:
        depth_image, color_image, gray_img, gray1 = process_frames(pipeline, align)
        Objects = identify_objects(gray_img, gray1)
        r1, c1 = pick_object(obj, Objects)
        # What's Left: Transferring Camera Frame to Robot Frame
        x = r1 - 580
        y = 150 - c1
        x_m = -0.00115
        y_m = 0.0011
        ...
        if obj == 'soft':
            x_b -= 0.3
            return x_b + (x * -x_m), y_b + (y * y_m)
            # return x_adjust + (x * -0.00115), y_adjust + (y * 0.0011), 0.69 - depth_image[c1, r1] / 1000 + 0.045

        elif obj == 'rigid':
            return x_b + (x * -x_m), y_b + (y * y_m)
            # return x_adjust + (x * -0.00114), y_adjust + (y * 0.0011), 0.8 - depth_image[c1, r1] / 950 - 0.045

    finally:
        pipeline.stop()


def plot_camera(gray_img, image_depth):
...
def process_frames(pipeline, align):
    ...
    return depth_image, color_image, gray_img, gray1
...
def identify_objects(gray_img, gray1):
    ...
    for pic, contourred in enumerate(contoursred1):
        ...
        if (area > 0):
            ...
            Objects['x1'].append(np.mean(x+x+w))
            Objects['y1'].append(y+y+h)
...
# plot_camera(gray_img, image_depth)
